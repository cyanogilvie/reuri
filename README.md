...
- Cyan Ogilvie
date: 0.16.0
title: reuri(3) 0.16.0 \| URI Manipulation for Tcl
---
...
A range iterates in whichever direction its resolved endpoints land —
the *written* direction is not consulted. So a range like “1..end” on a
single-element list resolves to “from=1, to=0” and silently descends,
yielding indices 1, 0 (one out-of-range element plus the sole element)
rather than an empty list. Likewise “5..end” on a three-element list
descends 5, 4, 3, 2 rather than clipping to empty. If you want
*lrange*-style “empty when out-of-bounds” semantics, guard on the list
...
https://github.com/cyanogilvie/dedup. Meson finds `dedup` via its
pkg-config file — set `PKG_CONFIG_PATH` to point meson at it (and Tcl)
...
release](https://github.com/cyanogilvie/reuri/releases/download/v0.16.0/reuri-0.16.0.tar.xz),
then build with meson:

``` sh
wget https://github.com/cyanogilvie/reuri/releases/download/v0.16.0/reuri-0.16.0.tar.xz
tar xf reuri-0.16.0.tar.xz
cd reuri-0.16.0
...
recursively, then build with meson:
...
``` dockerfile
WORKDIR /tmp/reuri
RUN wget https://github.com/cyanogilvie/reuri/releases/download/v0.16.0/reuri-0.16.0.tar.xz -O - | tar xJ --strip-components=1 && \
    meson setup builddir --buildtype=release && \
    meson install -C builddir && \
...
`-Dbuildtype=debug -Db_sanitize=address` — or use a PURIFY-enabled Tcl
runtime):
